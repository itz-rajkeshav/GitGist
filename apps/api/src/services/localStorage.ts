...
function getRepoStorageDir(repoUrl: string, config: StorageConfig): string {
  const sanitizedName = sanitizeRepoName(repoUrl);
  
  if (config.createTimestampFolders) {
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    return path.join(config.baseDir, sanitizedName, timestamp);
  }
  
  return path.join(config.baseDir, sanitizedName);
}

export async function saveFileAnalysis(repoUrl: string, analysis: FileAnalysis, config: StorageConfig): Promise<string> {
  try {
    const repoDir = getRepoStorageDir(repoUrl, config);
...
export async function saveMultipleFileAnalyses(repoUrl: string, analyses: FileAnalysis[], config: StorageConfig): Promise<{
  savedFiles: string[];
  summary: any;
}> {
  try {
    const existingRepos = await listStoredRepositories(config);
    ...
    for (const analysis of analyses) {
      const savedPath = await saveFileAnalysis(repoUrl, analysis, config);
...
function createSummary(analyses: FileAnalysis[]): any {
  const summary = {
    totalFiles: analyses.length,
    totalFunctions: 0,
    totalImports: 0,
...
export async function loadFileAnalysis(filePath: string): Promise<FileAnalysis> {
  try {
...
async function findLatestAnalysisFolder(repoUrl: string, config: StorageConfig): Promise<string> {
  try {
    const sanitizedName = sanitizeRepoName(repoUrl);
    const repoDir = path.join(config.baseDir, sanitizedName);
...
export async function loadRepoAnalyses(repoUrl: string, config: StorageConfig): Promise<FileAnalysis[]> {
  try {
    const latestFolder = await findLatestAnalysisFolder(repoUrl, config);
    ...
    const scanForAnalyses = async (dir: string): Promise<void> => {
      ...
      for (const entry of entries) {
        ...
        if (stat.isDirectory()) {
          ...
          try {
            const analysis = await loadFileAnalysis(entryPath);
...
export async function listStoredRepositories(config: StorageConfig): Promise<string[]> {
  try {
    if (!(await fs.pathExists(config.baseDir))) {
      return [];
    }
    
    const entries = await fs.readdir(config.baseDir);
    const repos: string[] = [];
    
    for (const entry of entries) {
      const entryPath = path.join(config.baseDir, entry);
      const stat = await fs.stat(entryPath);
